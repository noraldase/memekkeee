...
const app = $('#app');
let provider, account = '', chainOk = false, busy = false, logoFile = null, logoPreview = '', logoUploading = false, balances = { eth: null, pair: null }, balanceLoading = false, balanceError = '';
...
function formatBalance(value, decimals=6){ if(value===null||value===undefined)return '—'; const n=Number(value)/10**decimals; if(!Number.isFinite(n))return '—'; return n.toLocaleString(undefined,{maximumFractionDigits:6}); }
async function refreshBalances(){
 if(!provider||!account||!chainOk){balances={eth:null,pair:null};balanceError='';render();return}
 const pair=PAIRS.find(x=>x.symbol===form.pair)||PAIRS[0]; balanceLoading=true; balanceError=''; render();
 try{
  const ethRaw=await provider.request({method:'eth_getBalance',params:[account,'latest']});
  let pairRaw=ethRaw;
  if(!pair.native){ const data=new Interface(erc20).encodeFunctionData('balanceOf',[account]); pairRaw=await provider.request({method:'eth_call',params:[{to:pair.address,data},'latest']}); }
  balances={eth:BigInt(ethRaw),pair:BigInt(pairRaw)};
 }catch(error){ balances={eth:null,pair:null}; balanceError='Balance unavailable from wallet provider.'; }
 balanceLoading=false; render();
}
function scheduleBalanceRefresh(){refreshBalances().catch(()=>{balanceLoading=false;balanceError='Balance unavailable from wallet provider.';render()});}
 function render(){
...
async function launchOnPons(){
 ...
 const logoData=form.logo||await uploadLogo(logoFile);
...
function bind(){
 ...
 $('#launchForm')?.addEventListener('input',e=>{if(e.target.name){form[e.target.name]=e.target.value; if(e.target.name==='pair'){balances.pair=null;render();scheduleBalanceRefresh()} else if(e.target.name==='tax')render(); else { $('.token-name')&&( $('.token-name').textContent=form.name||'YOUR TOKEN NAME'); $('.token-ticker')&&( $('.token-ticker').textContent=form.ticker?'$'+form.ticker.replace(/^\$/,''):'$TICKER'); $('.token-card p')&&($('.token-card p').textContent=form.description||'A token identity begins here. Connect a wallet to shape the market.'); }}});
 $('#launchForm')?.addEventListener('change',e=>{if(e.target.name==='logo'&&e.target.files?.[0]){const file=e.target.files[0];if(!['image/png','image/jpeg','image/webp'].includes(file.type)){setMsg('Use PNG, JPG, or WEBP.','bad');return}if(file.size>5242880){setMsg('Logo must be under 5 MB.','bad');return}uploadLogo(file).catch(err=>{logoFile=null;logoPreview='';logoUploading=false;form.logo='';render();setMsg(err.message||'Logo upload failed.','bad')});} });
...
window.ethereum?.on?.('accountsChanged',()=>connect().catch(()=>{}));window.ethereum?.on?.('chainChanged',()=>connect().catch(()=>{}));window.ethereum?.on?.('message',e=>{if(e?.type==='eth_subscription')scheduleBalanceRefresh()});render();
